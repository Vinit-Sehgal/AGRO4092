...
```{r 12a2, message = FALSE, warning = FALSE}
 ...
 EFmx=0.650               # Max Evaporative fraction 

 por_s=0.5                # Surface field capacity
 wp_s=0.12                # Surface wilting point
 b_s=5                    # Power coefficient of surface drainage curve  
 Ks_s=1500                  # Saturated hydraulic conductivity of surface 
 
 por_r=0.5                # RZ field capacity
 wp_r=0.30                # RZ wilting point
 b_r=5                    # Power coefficient of rootzone drainage curve  
 Ks_r=200                 # Saturated hydraulic conductivity of rootzone 
 ...
 for (t in 2:length(PCP)){
   ...
   storageCap_s=(por_s-SSM[t-1])*zs
   SSM[t]=SSM[t-1]+pmin(PCP[t],storageCap_s)
   runoff[t]=pmax(0,(PCP[t]-(SSM[t]*zs)))
   
   # 1. Infiltration based on SSM state after runoff
   infilt[t]= Ks_s*(pmax(0,SSM[t-1]-cp_s))^b_s
   #pmax(wp_s,(zs*SSM[t-1]+PCP[t]-infilt[t]-runoff[t])/zs)
   ...
   # 1 Runoff: if SSM > por. Update SSM to por after runoff
   runoff[t]=PCP[t]-infilt[t]
   
   # # 3. Update SSM based on infilt and PCP
   # SSM[t]=ifelse(runoff[t]>0,cp_s,SSM[t-1])
   ...
   SSM[t]=ifelse(!(PCP[t]>0),
                 pmax(wp_s,(zs*SSM[t-1]-Es[t])/zs),SSM[t-1])
...
set.seed(100);ParOut=hydromad::SCEoptim(error_fun,                 
       ...
       lower = c(0.5,0.3,
                 0.25,0.08,1.01,20,
                 0.25,0.1,1.01,20),     # Lower values of params
       upper = c(2,0.95,
                 0.6,0.4,5,500,
