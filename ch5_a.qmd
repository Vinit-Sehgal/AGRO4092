...
For this assessment, we will use Aridity index (AI), which is a popular approach for climate classification based on the relative availability of Mean Annual Precipitation (***P***) compared to the Mean Annual Reference Evapotranspiration (***ET~0~***) of a location.
...
Do we agree that most pixels in the raster range within 0-5?
...
To explore the AI–SM relationship, first we will use the `terra::resample` function to resample the pixels in the `AI` raster to match the spatial resolution of `sm` pixels using `bilinear` interpolation.
This method estimates new cell values as a weighted-average values of the adjoining pixels.
The weights are calculated according to the distance of the centriod of the target pixel from the adjoining cells.
In addition to the `bilinear` approach, `terra::resample` has several interpolation methods available as options such as `near` (for nearest neighbor interpolation), `cubic`, `sum`, `min`, `max`, `average` , rms (root-mean square) etc.
...
We see that the soil moisture increases as AI increases before reaching an asymptotic value as AI\>0.65 (humid climate, indicated by red vertical line in the plot).
...
> **Food for thought**: Can we do this analysis had we used `AI` raster instead of `aridityResamp`?
> What will be the output is we replace `aridityResamp` with `AI` in the code above.
...
In the AI plot above, we see the expected climate pattern for terrestrial landmass.
The equatorial regions receive abundant precipitation, for example the Amazon forest and Southeast Asia, and have high aridity index values.
Regions like Sahara desert, Central Australia, Western US are have hot and dry climate, and show low values of the aridity index.
...
A discrete attribute table can be added to a raster which serves as a look-up reference for discrete classification of the continuous variable in the raster.
...
As an example, we will follow class breaks and names given in **Table 1** to add climate attribute to the `AI` raster.
We will `cut` the pixel `values` of the AI raster into discrete classes and ass the attribute back to the original AI raster.
...
attributes=base::cut(values(AI), # Notice that we apply cut after extracting raster values. 
                 breaks = class_brk,
                 labels =class_names )
...
In the subsequent examples, we will use the aridity index raster to create a polygon, demarcating the spatial boundaries of different climate zones.
...
We will also transform the aridity raster to a polygon using the function `as.polygons` to find the mean soil moisture values for each aridity class.
...
Now we will use `terra::extract` function to extract the cell values of the soil moisture raster within each climate zone.
...
sm_climate_median=terra::extract(sm,    # Raster to be summarized
                          arid_poly,    # Shapefile/ polygon to summarize the raster
                          fun=median,   # Desired statistic: median, mean, sum, min and max, etc.? 
                          na.rm=TRUE)   # Ignore NA values? yes! 
...
This is a good point to learn about another excellent function for raster cell extraction from the `exactextractr` package, `exact_extract`.
This function is computationally better than `terra::extract`, which will be evident when working with rasters with heavier size.

> **Remember**: `exactextractr::exact_extract` also outputs the fractional cell coverage of each pixel extracted.
> For several analysis, this information may not be needed.
...
Cell value extraction for each zone can help a user to carry a diver set of analysis for each region using user defined function.
...
Here, we will use the RATified AI raster to extract cell values corresponding to each zone.

```{r 5a10, message = FALSE, warning = FALSE}
zonal_data=list()
climate_num=as.numeric(AI$climate)
climate_num=resample(climate_num,sm, method="near")

# Extract pixel values within each climate zone
zonal_data=lapply(list(1,2,3,4,5),function(x) (na.omit((sm[climate_num==x]))))

# Calculate and store stats for each climate zone
zonal_stat_list=lapply(list(1,2,3,4,5),function(x) (mean(sm[climate_num==x], na.rm=TRUE)))
