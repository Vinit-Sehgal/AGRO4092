...
sm2=sm+1
print(sm2, main= "sm+1") #Notice the max and min values have increased by 1. 

# Multiply raster values by 2
sm2=sm*2
plot(sm2, main= "sm*+1") # Try sm2=sm*10, or sm2=sm^2 and see the difference in sm2 values

# Similarly, logical operations can also be carried out with ease.
sm2=sm>0.3   # Are cell values of sm raster greater than 0.3?
plot(sm2)    # Notice that the raster has only True and False values

# Logical operations can also be a convenient tool for raster data manipulation 
sm[sm>0.3]=0.5 
plot(sm)
```
...
Imagine a raster may have a fill value of -999, it may be advisable to remove these values by replacing them with `NA` in certain cases.
